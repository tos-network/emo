...
func TestDHTBootstrapSetAndFind(t *testing.T) {
	bootstrapCfg := &Config{
		LocalID:       randomID(),
	...
	bootstrapDHT, err := New(bootstrapCfg)
	...
	go func() {
		...
		bootstrapDHT.Store(key, value, time.Hour, func(err error) {
			storeDone <- err
		})
		...
		bootstrapDHT.Find(key, func(v []byte, err error) {
			if err == nil {
				retrievedValue = make([]byte, len(v))
				copy(retrievedValue, v)
			}
			findDone <- err
		})

		require.NoError(t, <-findDone)
	...
	// Clean up
	require.NoError(t, bootstrapDHT.Close())
}
...
func BenchmarkDHTLocalStore(b *testing.B) {
	...
	dht.routing.insert(c.LocalID, addr, time.Duration(0), false)
...
func TestDHTKeyRefresh(t *testing.T) {
	...
	bootstrapDHT.Store(key, value, time.Minute, func(err error) {
		done <- err
	})
	...
	// Verify the value is still available
	bootstrapDHT.Find(key, func(v []byte, err error) {
		require.NoError(t, err)
...
func TestLatencyBasedRouting(t *testing.T) {
	...
	// Add test nodes with different latencies
	for i := 0; i < 10; i++ {
		node := &node{
			id:       randomID(),
			address:  &net.UDPAddr{IP: net.ParseIP("127.0.0.1"), Port: 9001 + i},
			latency:  time.Duration(i*100) * time.Millisecond,
			testMode: true,
		}
		dht.routing.insert(node.id, node.address, node.latency, node.testMode)
	}

	// Allow some time for routers to measure latency
	time.Sleep(time.Millisecond * 500)

	// Test that route selection prefers lower latency nodes
	routes := dht.latencyRouter.GetBestRoutes(randomID(), 3)
	require.Len(t, routes, 3)

	// Verify latency ordering
	for i := 1; i < len(routes); i++ {
		assert.True(t, routes[i-1].latency <= routes[i].latency)
	}
}
