...
If you just want to install emo, please download it from https://dist.ipfs.tech/#emo. This document explains how to build it from source.

## Install Go
`emo` is built on Golang and thus depends on it for all building methods.  
https://golang.org/doc/install  
The `GOPATH` environment variable must be set as well.  
https://golang.org/doc/code.html#GOPATH

## Choose the way you want to proceed
`emo` utilizes `make` to automate builds and run tests, but can be built without it using only `git` and `go`.  
No matter which method you choose, if you encounter issues, please see the [Troubleshooting](#troubleshooting) section.  

**Using `make`:**  
MSYS2 and Cygwin provide the Unix tools we need to build `emo`. You may use either, but if you don't already have one installed, we recommend MSYS2.  
[MSYS2→](#msys2)  
[Cygwin→](#cygwin)  
...
go get -u github.com/ipfs/emo
cd %GOPATH%\src\github.com\ipfs\emo
make install
%GOPATH%\bin\ipfs.exe version --all
```

If there were no errors, the final command should output version information similar to "`ipfs version 0.4.14-dev-XXXXXXX`" where "XXXXXXX" should match the current short-hash of the `emo` repo. You can retrieve said hash via this command: `git rev-parse --short HEAD`.  
...
|`pacman --noconfirm -S  git make unzip`   |Install `emo` build dependencies|
|`go get -u github.com/ipfs/emo`       |Fetch / Update `emo` source|
|`cd %GOPATH%\src\github.com\ipfs\emo` |Change to `emo` source directory|
...
|`mkdir %GOPATH%\src\github.com\ipfs`<br/>`cd %GOPATH%\src\github.com\ipfs`<br/>`git clone https://github.com/ipfs/emo.git`       |Fetch / Update `emo` source|
|`cd %GOPATH%\src\github.com\ipfs\emo` |Change to `emo` source directory|
|`make install`                            |Build and install to `%GOPATH%\bin\ipfs.exe`|
|`%GOPATH%\bin\ipfs.exe version --all`     |Test the built binary|
...
While it's possible to build `emo` with `go` alone, we'll be using `git` to fetch the source.
...
CMD:
```bat
git clone https://github.com/ipfs/emo %GOPATH%/src/github.com/ipfs/emo
cd %GOPATH%/src/github.com/ipfs/emo/cmd/ipfs
```

PowerShell:
```powershell
git clone https://github.com/ipfs/emo $env:GOPATH/src/github.com/ipfs/emo
cd $env:GOPATH/src/github.com/ipfs/emo/cmd/ipfs
```
...
CMD:
```bat
go install -ldflags="-X "github.com/ipfs/emo".CurrentCommit=%SHA%"
%GOPATH%\bin\ipfs.exe version --all
```

PowerShell:
```powershell
go install -ldflags="-X "github.com/ipfs/emo".CurrentCommit=$env:SHA"
...
If you encounter a bug with `emo` itself (not related to building) please use the [issue tracker](https://github.com/ipfs/emo/issues) to report it.
