<!-- Last updated during [v0.30.0 release](https://github.com/ipfs/emo/pull/10496) -->
...
- [ ] Verify you have access to all the services and tools required for the release
  ...
  - [ ] [emoreleaser](https://github.com/ipfs/emoreleaser) checked out on your system (_only if you're using [emoreleaser](https://github.com/ipfs/emoreleaser)_)
  ...
  - [ ] [docker](https://docs.docker.com/get-docker/) installed on your system (_only if you're using [emoreleaser](https://github.com/ipfs/emoreleaser)_)
  - [ ] [npm](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm) installed on your system (_only if you're **NOT** using [emoreleaser](https://github.com/ipfs/emoreleaser)_)
  - [ ] [zsh](https://github.com/ohmyzsh/ohmyzsh/wiki/Installing-ZSH#install-and-set-up-zsh-as-default) installed on your system
  - [ ] [emo](https://github.com/ipfs/emo) checked out under `$(go env GOPATH)/src/github.com/ipfs/emo`
    - you can also symlink your clone to the expected location by running `mkdir -p $(go env GOPATH)/src/github.com/ipfs && ln -s $(pwd) $(go env GOPATH)/src/github.com/ipfs/emo`
...
- [ ] Prepare the release branch and update version numbers accordingly <details><summary>using `./emoreleaser --skip-check-before release --version vX.Y.Z(-rcN) prepare-branch` or ...</summary>
  ...
  - [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) Add full changelog and contributors to the [changelog](docs/changelogs/vX.Y.md)
    - [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) Replace the `Changelog` and `Contributors` sections of the [changelog](docs/changelogs/vX.Y.md) with the stdout of `./bin/mkreleaselog`. Note that the command expects your `$GOPATH/src/github.com/ipfs/emo` to include latest commits from `release-vX.Y`
...
- [ ] Create the release tag <details><summary>using `./emoreleaser release --version vX.Y.Z(-rcN) tag` or ...</summary>
...
- [ ] Verify [ipfs/distributions](https://github.com/ipfs/distributions)'s `.tool-versions`'s `golang` entry is set to the [latest go release](https://go.dev/doc/devel/release) on the major go branch [Emo is being tested on](https://github.com/ipfs/emo/blob/master/.github/workflows/gotest.yml) (see `go-version:`).
- [ ] Publish to Dockerhub, NPM, and dist.ipfs.tech and GitHub using `./emoreleaser --skip-check-before --skip-run release --version vX.Y.Z(-rcN) publish-to-all` or follow each step below:
  - [ ] Publish the release to [DockerHub](https://hub.docker.com/r/ipfs/emo/) <details><summary>using `./emoreleaser --skip-check-before --skip-run release --version vX.Y.Z(-rcN) publish-to-dockerhub` or ...</summary>
    - [ ] Wait for [Publish docker image](https://github.com/ipfs/emo/actions/workflows/docker-image.yml) workflow run initiated by the tag push to finish
    - [ ] verify the image is available on [Docker Hub](https://hub.docker.com/r/ipfs/emo/tags)
  - [ ] Publish the release to [dist.ipfs.tech](https://dist.ipfs.tech) <details><summary>using `./emoreleaser release --version vX.Y.Z(-rcN) publish-to-distributions` or ...</summary>
    - [ ] check out [ipfs/distributions](https://github.com/ipfs/distributions)
    - [ ] create new branch: run `git checkout -b release-emo-X.Y.Z(-rcN)` 
    - [ ] run `./dist.sh add-version emo vX.Y.Z(-RCN)` to add the new version to the `versions` file
      - [usage](https://github.com/ipfs/distributions#usage)
    - [ ] create and merge the PR which updates `dists/emo/versions` and `dists/go-ipfs/versions` (![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) and `dists/emo/current` and `dists/go-ipfs/current`)
  ...
  - [ ] Publish the release to [NPM](https://www.npmjs.com/package/emo?activeTab=versions) <details><summary>using `./emoreleaser release --version vX.Y.Z(-rcN) publish-to-npm` (⚠️ you might need to run the command a couple of times because GHA might not be able to see the new distribution straight away due to caching) or ...</summary>
    ...
    - [ ] verify the release is available on [NPM](https://www.npmjs.com/package/emo?activeTab=versions)
    </details>
  - [ ] Publish the release to [GitHub](https://github.com/ipfs/emo/releases) <details><summary>using `./emoreleaser release --version vX.Y.Z(-rcN) publish-to-github` or ...</summary>
    - [ ] create a new release on [GitHub](https://docs.github.com/en/repositories/releasing-projects-on-github/managing-releases-in-a-repository#creating-a-release)
      - [RC example](https://github.com/ipfs/emo/releases/tag/v0.17.0-rc1)
      - [FINAL example](https://github.com/ipfs/emo/releases/tag/v0.17.0)
    ...
    - [ ] run the [sync-release-assets](https://github.com/ipfs/emo/actions/workflows/sync-release-assets.yml) workflow
    - [ ] wait for the [sync-release-assets](https://github.com/ipfs/emo/actions/workflows/sync-release-assets.yml) workflow run to finish
    - [ ] verify the release assets are present in the [GitHub release](https://github.com/ipfs/emo/releases/tag/vX.Y.Z(-RCN))
    </details>
- [ ] Update Emo staging environment, see the [Running Emo tests on staging](https://www.notion.so/Running-Emo-tests-on-staging-488578bb46154f9bad982e4205621af8) for details.
...
- [ ] Promote the release <details><summary>using `./emoreleaser release --version vX.Y.Z(-rcN) promote` or ...</summary>
  - [ ] create an [IPFS Discourse](https://discuss.ipfs.tech) topic
    - [prerelease example](https://discuss.ipfs.tech/t/emo-v0-16-0-rc1-release-candidate-is-out/15248)
    - [release example](https://discuss.ipfs.tech/t/emo-v0-16-0-release-is-out/15249)
    - [ ] use `Emo vX.Y.Z(-RCN) is out!` as the title
    - [ ] use `emo` and `go-ipfs` as topics
  ...
  - [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) Add the link to the [IPFS Discourse](https://discuss.ipfs.tech) topic to the [GitHub Release](https://github.com/ipfs/emo/releases/tag/vX.Y.Z(-RCN)) description
    - [example](https://github.com/ipfs/emo/releases/tag/v0.17.0)
  - [ ] ![](https://img.shields.io/badge/only-RC-blue?style=flat-square) create an issue comment mentioning early testers on the release issue
    - [example](https://github.com/ipfs/emo/issues/9319#issuecomment-1311002478)
  - [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) create an issue comment linking to the release on the release issue
    - [example](https://github.com/ipfs/emo/issues/9417#issuecomment-1400740975)
  ...
  - [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) ![](https://img.shields.io/badge/not-PATCH-yellow?style=flat-square) post the link to the [GitHub Release](https://github.com/ipfs/emo/releases/tag/vX.Y.Z(-RCN)) to [Reddit](https://reddit.com/r/ipfs)
    - [example](https://www.reddit.com/r/ipfs/comments/9x0q0k/emo_v0160_release_is_out/)
  </details>
- [ ] ~~Test the new version with `ipfs-companion`~~ ([currently skipped](https://github.com/ipfs/ipfs-companion/issues/1300)) <details><summary>using `./emoreleaser release --version vX.Y.Z(-rcN) test-ipfs-companion` or ...</summary>
  - [ ] run the [e2e](https://github.com/ipfs/ipfs-companion/actions/workflows/e2e.yml)
    - use `vX.Y.Z(-RCN)` as the Emo image version
  - [ ] wait for the [e2e](https://github.com/ipfs/ipfs-companion/actions/workflows/e2e.yml) workflow run to finish
  </details>
- [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) Update Emo in [ipfs-desktop](https://github.com/ipfs/ipfs-desktop) <details><summary>using `./emoreleaser release --version vX.Y.Z(-rcN) update-ipfs-desktop` or ...</summary>
...
- [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) Update Emo docs <details><summary>using `./emoreleaser release --version vX.Y.Z(-rcN) update-ipfs-docs` or ...</summary>
...
- [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) Create a blog entry on [blog.ipfs.tech](https://blog.ipfs.tech) <details><summary>using `./emoreleaser release --version vX.Y.Z(-rcN) update-ipfs-blog --date YYYY-MM-DD` or ...</summary>
  - [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) create a PR which adds a release note for the new Emo version
...
- [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) Merge the [release](https://github.com/ipfs/emo/tree/release) branch back into [master](https://github.com/ipfs/emo/tree/master), ignoring the changes to [version.go](version.go) (keep the `-dev`) version, <details><summary>using `./emoreleaser release --version vX.Y.Z(-rcN) merge-branch` or ...</summary>
  - [ ] create a new branch `merge-release-vX.Y.Z` from `release`
  - [ ] create and merge a PR from `merge-release-vX.Y.Z` to `master`
  </details>
- [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) ![](https://img.shields.io/badge/not-PATCH-yellow?style=flat-square) Prepare for the next release <details><summary>using `./emoreleaser release --version vX.Y.Z(-rcN) prepare-next` or ...</summary>
  - [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) ![](https://img.shields.io/badge/not-PATCH-yellow?style=flat-square) Create the next [changelog](https://github.com/ipfs/emo/blob/master/docs/changelogs/vX.(Y+1).md)
...
- [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) ![](https://img.shields.io/badge/not-PATCH-yellow?style=flat-square) Create a dependency update PR
  - [ ] ![](https://img.shields.io/badge/only-FINAL-green?style=flat-square) ![](https://img.shields.io/badge/not-PATCH-yellow?style=flat-square) check out [ipfs/emo](https://github.com/ipfs/emo)
