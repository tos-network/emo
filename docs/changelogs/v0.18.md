...
IF you run a server and want to utilize more than half of memory and file descriptors to p2p work, adjust [`Swarm.ResourceMgr.MaxMemory`](https://github.com/ipfs/emo/blob/master/docs/config.md#swarmresourcemgrmaxmemory) and [`Swarm.ResourceMgr.MaxFileDescriptors`](https://github.com/ipfs/emo/blob/master/docs/config.md#swarmresourcemgrmaxfiledescriptors).

The 0.18.1 builds on the default protection nodes get against DoS (resource exhaustion) and eclipse attacks
with the [go-libp2p Network Resource Manager/Accountant](https://github.com/ipfs/emo/blob/master/docs/libp2p-resource-management.md)
that was fine-tuned in [Emo 0.18](https://github.com/ipfs/emo/blob/biglep/resource-manager-example-of-what-want/docs/changelogs/v0.18.md#improving-libp2p-resource-management-integration).

Adding default hard-limits from the Resource Manager/Accountant after the fact is tricky,
and some additional improvements have been made to improve the [computed defaults](https://github.com/ipfs/emo/blob/master/docs/libp2p-resource-management.md#computed-default-limits).
...
More updates are likely in future Emo releases, but with this release: 
...
- github.com/ipfs/emo:
  ...
  - feat: Pubsub.SeenMessagesStrategy (#9543) ([ipfs/emo#9543](https://github.com/ipfs/emo/pull/9543))
...
Content routing is the process of discovering which peers provide a piece of content. Emo has traditionally only supported [libp2p's implementation of Kademlia DHT](https://github.com/libp2p/specs/tree/master/kad-dht) for content routing.

Emo can now bridge networks by including support for the [delegated routing HTTP API](https://github.com/ipfs/specs/pull/337). Users can compose content routers using the `Routing.Routers` config to pick content routers with different tradeoffs than a Kademlia DHT (e.g., high-performance and high-capacity centralized endpoints, dedicated Kademlia DHT nodes, routers with unique provider records, privacy-focused content routers).

One example is [InterPlanetary Network Indexers](https://github.com/ipni/specs/blob/main/IPNI.md#readme), which are HTTP endpoints that cache records from both the IPFS network and other sources such as web3.storage and Filecoin. This improves not only content availability by enabling Emo to transparently fetch content directly from Filecoin storage providers, but also improves IPFS content routing latency by an order of magnitude and decreases resource consumption.

> *Note:* it's possible to retrieve content stored by Filecoin Storage Providers (SPs) from Emo if the SPs service Bitswap requests.  As of this release, some SPs are advertising Bitswap.  You can follow the roadmap progress for IPNIs and Bitswap in SPs [here](https://www.starmaps.app/roadmap/github.com/protocol/bedrock/issues/1).
...
Learn more in the [`Routing` docs](https://github.com/ipfs/emo/blob/master/docs/config.md#routing).
...
[emo#9326](https://github.com/ipfs/emo/pull/9326),
...
since Emo 0.13, but in this release it will also include the size column.

#### QUIC and WebTransport

##### WebTransport enabled by default
[WebTransport](https://docs.libp2p.io/concepts/transports/webtransport/) is a new libp2p transport that [was introduced in v0.16](https://github.com/ipfs/emo/blob/master/docs/changelogs/v0.16.md#-webtransport-new-experimental-transport) that is based on top of QUIC and HTTP3.

This allows browser-based nodes to contact Emo nodes, so now instead of just serving requests for other system-level application nodes, you can also serve requests directly to a node running inside a browser page.
...
Support for QUIC Draft-29 will be removed at some point in 2023 ([tracking issue](https://github.com/ipfs/emo/issues/9496)).  As a result, new deployments should use `/quic-v1` instead of `/quic`.
...
Emo enabled the [go-libp2p Network Resource Manager](https://github.com/libp2p/go-libp2p/tree/master/p2p/host/resource-manager)
by default in [Emo 0.17](https://github.com/ipfs/emo/blob/master/docs/changelogs/v0.17.md#libp2p-resource-management-enabled-by-default).
...
1. [Dedicated docs concerning the resource manager integration](https://github.com/ipfs/emo/blob/master/docs/libp2p-resource-management.md).  This is a great place to go to learn more or get your FAQs answered.
2. Increasing the default limits for the resource manager.
3. Enabling the [`Swarm.ConnMgr`](https://github.com/ipfs/emo/blob/master/docs/config.md#swarmconnmgr) by default and reducing it thresholds so it can intelligently prune connections in many cases before the indiscriminate resource manager kicks in.
...
5. [Other miscellaneous config and command bugs reported by users](https://github.com/ipfs/emo/issues/9442).
...
- github.com/ipfs/emo:
  - fix: clarity: no user supplied rcmgr limits of 0 (#9563) ([ipfs/emo#9563](https://github.com/ipfs/emo/pull/9563))
  - fix(gateway): undesired conversions to dag-json and friends (#9566) ([ipfs/emo#9566](https://github.com/ipfs/emo/pull/9566))
  ...
  - docs: improve docs/README (#9539) ([ipfs/emo#9539](https://github.com/ipfs/emo/pull/9539))
  - test: port CircleCI to GH Actions and improve sharness reporting (#9355) ([ipfs/emo#9355](https://github.com/ipfs/emo/pull/9355))
  - chore: migrate from go-ipfs-files to go-libipfs/files (#9535) ([ipfs/emo#9535](https://github.com/ipfs/emo/pull/9535))
  - fix: stats dht command when Routing.Type=auto (#9538) ([ipfs/emo#9538](https://github.com/ipfs/emo/pull/9538))
  - fix: hint people to changing from RSA peer ids
  - fix(gateway): JSON when Accept is a list
  - fix(test): retry flaky t0125-twonode.sh
  - docs: fix Router config Godoc (#9528) ([ipfs/emo#9528](https://github.com/ipfs/emo/pull/9528))
  - fix(ci): flaky sharness test
  - docs(config): ProviderSearchDelay (#9526) ([ipfs/emo#9526](https://github.com/ipfs/emo/pull/9526))
  ...
  - Removing QRI from early tester ([ipfs/emo#9503](https://github.com/ipfs/emo/pull/9503))
  - fix: disable provide over HTTP with Routing.Type=auto (#9511) ([ipfs/emo#9511](https://github.com/ipfs/emo/pull/9511))
  - Update version.go
  - 'chore: update version.go'
  - Clened up 0.18 changelog for release ([ipfs/emo#9497](https://github.com/ipfs/emo/pull/9497))
  - feat: turn on WebTransport by default ([ipfs/emo#9492](https://github.com/ipfs/emo/pull/9492))
  - feat: fast directory listings with DAG Size column (#9481) ([ipfs/emo#9481](https://github.com/ipfs/emo/pull/9481))
  - feat: add basic CLI tests using Go Test
  - Changelog: v0.18.0 ([ipfs/emo#9485](https://github.com/ipfs/emo/pull/9485))
  ...
  - docs(readme): update package managers section (#9488) ([ipfs/emo#9488](https://github.com/ipfs/emo/pull/9488))
  - fix: support /quic-v1 in webui v0.21
  - feat: Routing.Type=auto (DHT+IPNI) (#9475) ([ipfs/emo#9475](https://github.com/ipfs/emo/pull/9475))
  - feat: adjust ConnMgr target to 32-96 (#9483) ([ipfs/emo#9483](https://github.com/ipfs/emo/pull/9483))
  - feat: increase default Reprovider.Interval (#9326) ([ipfs/emo#9326](https://github.com/ipfs/emo/pull/9326))
  - feat: add response body limiter to routing HTTP client (#9478) ([ipfs/emo#9478](https://github.com/ipfs/emo/pull/9478))
  - docs: libp2p resource management (#9468) ([ipfs/emo#9468](https://github.com/ipfs/emo/pull/9468))
  - chore: upgrade libipfs for routing HTTP API schema changes (#9477) ([ipfs/emo#9477](https://github.com/ipfs/emo/pull/9477))
  ...
  - feat(gateway): JSON and CBOR response formats (IPIP-328) (#9335) ([ipfs/emo#9335](https://github.com/ipfs/emo/pull/9335))
  -  ([ipfs/emo#9318](https://github.com/ipfs/emo/pull/9318))
  - docs: release process updates from v0.17.0 ([ipfs/emo#9391](https://github.com/ipfs/emo/pull/9391))
  - fix(rcmgr): improve error phrasing
  - docs: Update CHANGELOG.md adding 0.17 link
  - feat(config): Pubsub.SeenMessagesTTL (#9372) ([ipfs/emo#9372](https://github.com/ipfs/emo/pull/9372))
  - docs: remove snap and chocolatey packages
  - Merge release v0.17.0 ([ipfs/emo#9431](https://github.com/ipfs/emo/pull/9431))
  - docs: ipfs-http-client -> emo-rpc-client (#9331) ([ipfs/emo#9331](https://github.com/ipfs/emo/pull/9331))
  - docs(readme): improve tldr
  - Update config.md for resource management limits (#9421) ([ipfs/emo#9421](https://github.com/ipfs/emo/pull/9421))
  - Doc improvements and changelog for resource manager (#9413) ([ipfs/emo#9413](https://github.com/ipfs/emo/pull/9413))
  ...
  - docs: replace tabcat with aphelionz in EARLY_TESTERS.md (#9404) ([ipfs/emo#9404](https://github.com/ipfs/emo/pull/9404))
  - docs: fix spoiler for 0.13.1 changelog
  - fix(docs): typo
  - chore: bump version to v0.18.0-dev ([ipfs/emo#9393](https://github.com/ipfs/emo/pull/9393))
