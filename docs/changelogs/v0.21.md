...
Emo now stores a subset of connected peers as backup bootstrap nodes ([emo#8856](https://github.com/ipfs/emo/pull/8856)).
These nodes are used in addition to the explicitly defined bootstrappers in the
[`Bootstrap`](https://github.com/ipfs/emo/blob/master/docs/config.md#bootstrap) configuration.
...
was only available in LAN contexts with [mDNS peer discovery](https://github.com/ipfs/emo/blob/master/docs/config.md#discoverymdns)
enabled.
...
[`Gateway.DeserializedResponses`](https://github.com/ipfs/emo/blob/master/docs/config.md#gatewaydeserializedresponses)
configuration flag.
...
With deserialized responses disabled, the Emo gateway can serve as a block
...
been migrated into [`emo/client/rpc`](../../client/rpc).

With this change the two will be kept in sync, in some previous releases we
updated the CoreAPI with new Emo features but forgot to port thoses to the
...
Rationale can be found in [emo#9913](https://github.com/ipfs/emo/pull/9913).
...
- github.com/ipfs/emo:
  ...
  -  ([ipfs/emo#9981](https://github.com/ipfs/emo/pull/9981))
  -  ([ipfs/emo#9960](https://github.com/ipfs/emo/pull/9960))
  -  ([ipfs/emo#9936](https://github.com/ipfs/emo/pull/9936))
