...
Support for exposing the legacy subset of Emo RPC via the Gateway port is deprecated and should not be used. It will be removed in the next version. You can read more in <https://github.com/ipfs/emo/issues/10312>.
...
You can now configure the upper-bound of a cached IPNS entry's Time-To-Live via [`Ipns.MaxCacheTTL`](https://github.com/ipfs/emo/blob/master/docs/config.md#ipnsmaxcachettl).

#### RPC client: deprecated DHT API, added Routing API

The RPC client for GO (`emo/client/rpc`) now includes a Routing API to match the available commands in `/api/v0/routing`. In addition, the DHT API has been marked as deprecated.
...
All the DHT commands that were deprecated for over a year were finally removed from `/api/v0/dht`. Users should switch to modern `/api/v0/routing` which works with [both Amino DHT and Delegated Routers](https://github.com/ipfs/emo/blob/master/docs/config.md#routing).

#### Repository migrations are now trustless

Emo now only uses [trustless requests](https://specs.ipfs.tech/http-gateways/trustless-gateway/) (e.g., CAR files) when downloading repository migrations via HTTP. This further strengthens Emo by not delegating trust to public gateways. The migration binaries are locally verified before being executed. 
...
- github.com/ipfs/emo:
  ...
  - chore: update version
  - repo/fsrepo/migrations: verified HTTP migrations (#10324) ([ipfs/emo#10324](https://github.com/ipfs/emo/pull/10324))
  - chore: fix link
  - docs: clarify Gateway.ExposeRoutingAPI (#10337) ([ipfs/emo#10337](https://github.com/ipfs/emo/pull/10337))
  - commands/add: return an error when using --only-hash and --to-files
  - docs(config): mention routing v1 spec
  - core/commands: remove 'ipfs dht' commands, except 'query' (#10328) ([ipfs/emo#10328](https://github.com/ipfs/emo/pull/10328))
  - core: deprecate CoreAPI.Dht, introduce CoreAPI.Routing
  - refactor: superfluous namespace test redirects (#10322) ([ipfs/emo#10322](https://github.com/ipfs/emo/pull/10322))
  - feat: add Ipns.MaxCacheTTL
  - fix(gw): negative entity-bytes beyond file size (#10320) ([ipfs/emo#10320](https://github.com/ipfs/emo/pull/10320))
  ...
  - Merge Release: v0.26.0 [skip changelog] ([ipfs/emo#10313](https://github.com/ipfs/emo/pull/10313))
  - config: remove all options that are marked as REMOVED
  - chore: remove Gateway.APICommands
  - docs(cli): name inspect --verify (#10308) ([ipfs/emo#10308](https://github.com/ipfs/emo/pull/10308))
  - docs: improve release issue template (#10305) ([ipfs/emo#10305](https://github.com/ipfs/emo/pull/10305))
