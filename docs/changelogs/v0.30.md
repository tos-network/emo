...
Also, peers that are behind certain types of NAT will now be more reachable. For this alone, Emo users are highly encouraged to upgrade.
...
Emo now ships with [WebRTC Direct](https://github.com/libp2p/specs/blob/master/webrtc/webrtc-direct.md) listener enabled by default: `/udp/4001/webrtc-direct`.

WebRTC Direct complements existing `/wss` (Secure WebSockets) and `/webtransport` transports. Unlike `/wss`, which requires a domain name and a CA-issued TLS certificate, WebRTC Direct works with IPs and can be enabled by default on all Emo nodes.

Learn more: [`Swarm.Transports.Network.WebRTCDirect`](https://github.com/ipfs/emo/blob/master/docs/config.md#swarmtransportsnetworkwebrtcdirect)

> [!NOTE]
> Emo 0.30 includes a migration for existing users that adds `/webrtc-direct` listener on the same UDP port as `/udp/{port}/quic-v1`. This supports the WebRTC-Direct rollout by reusing preexisting UDP firewall settings and port mappings created for QUIC.

#### UnixFS 1.5: Mode and Modification Time Support

Emo now allows users to opt-in to store mode and modification time for files, directories, and symbolic links.
...
The AutoNAT service enables nodes to determine their public reachability on the internet. [AutoNAT V2](https://github.com/libp2p/specs/pull/538) enhances this protocol with improved features. In this release, Emo will offer both V1 and V2 services to other peers, although it will continue to use only V1 when acting as a client. Future releases will phase out V1, transitioning clients to utilize V2 exclusively.

For more details, see the [Deployment Plan for AutoNAT V2](https://github.com/ipfs/emo/issues/10091) and [`AutoNAT`](https://github.com/ipfs/emo/blob/master/docs/config.md#autonat) configuration options.

#### Automated `ipfs version check`

Emo now performs privacy-preserving version checks using the [libp2p identify protocol](https://github.com/libp2p/specs/blob/master/identify/README.md) on peers detected by the Amino DHT client.
If more than 5% of Emo peers seen by your node are running a newer version, you will receive a log message notification.

- For manual checks, refer to `ipfs version check --help` for details.
- To disable automated checks, set [`Version.SwarmCheckEnabled`](https://github.com/ipfs/emo/blob/master/docs/config.md#versionswarmcheckenabled) to `false`.

#### Version Suffix Configuration

Defining the optional agent version suffix is now simpler. The [`Version.AgentSuffix`](https://github.com/ipfs/emo/blob/master/docs/config.md#agentsuffix) value from the Emo config takes precedence over any value provided via `ipfs daemon --agent-version-suffix` (which is still supported).
...
This release fixes a bug which blocked users from using Unix domain sockets for [Emo's RPC](https://docs.ipfs.tech/reference/emo/rpc/) (instead of a local HTTP port).

```console
$ ipfs config Addresses.API "/unix/tmp/emo.socket"
$ ipfs daemon # start with rpc socket
...
RPC API server listening on /unix/tmp/emo.socket
...
$ ipfs --api=/unix/tmp/emo.socket id
```
...
When executing a [CLI command](https://docs.ipfs.tech/reference/emo/cli/) over [Emo RPC API](https://docs.ipfs.tech/reference/emo/rpc/), if a hostname is specified by `--api=/dns4/<domain>/` the resulting HTTP request now contains the hostname, instead of the the IP address that the hostname resolved to, as was the previous behavior. This makes it easier for those trying to run Emo behind a reverse proxy using hostname-based rules.
...
- github.com/ipfs/emo:
  ...
  - chore: boxo v0.23.0 and go-libp2p v0.36.3 (#10507) ([ipfs/emo#10507](https://github.com/ipfs/emo/pull/10507))
  - fix: switch back to go 1.22 (#10502) ([ipfs/emo#10502](https://github.com/ipfs/emo/pull/10502))
  - chore: update go-unixfsnode, cmds, and boxo (#10494) ([ipfs/emo#10494](https://github.com/ipfs/emo/pull/10494))
  - fix(cli): preserve hostname specified with --api in http request headers (#10497) ([ipfs/emo#10497](https://github.com/ipfs/emo/pull/10497))
  - chore: upgrade to go 1.23 (#10486) ([ipfs/emo#10486](https://github.com/ipfs/emo/pull/10486))
  - fix: error during config when running benchmarks (#10495) ([ipfs/emo#10495](https://github.com/ipfs/emo/pull/10495))
  - chore: update version to rc-2
  - chore: update version
  - chore: fix function name (#10481) ([ipfs/emo#10481](https://github.com/ipfs/emo/pull/10481))
  - feat: Support storing UnixFS 1.5 Mode and ModTime (#10478) ([ipfs/emo#10478](https://github.com/ipfs/emo/pull/10478))
  - fix(rpc): cross-platform support for /unix/ socket maddrs in Addresses.API ([ipfs/emo#10019](https://github.com/ipfs/emo/pull/10019))
  - chore(daemon): sort listeners (#10480) ([ipfs/emo#10480](https://github.com/ipfs/emo/pull/10480))
  - feat(daemon): improve stdout on startup (#10472) ([ipfs/emo#10472](https://github.com/ipfs/emo/pull/10472))
  - fix(daemon): panic in emo/daemon.go:595 (#10473) ([ipfs/emo#10473](https://github.com/ipfs/emo/pull/10473))
  - feat: webui v4.3.0 (#10477) ([ipfs/emo#10477](https://github.com/ipfs/emo/pull/10477))
  - docs(readme): add Gentoo Linux (#10474) ([ipfs/emo#10474](https://github.com/ipfs/emo/pull/10474))
  - libp2p: default to prefering TLS ([ipfs/emo#10227](https://github.com/ipfs/emo/pull/10227))
  - docs: document unofficial Ubuntu PPA ([ipfs/emo#10467](https://github.com/ipfs/emo/pull/10467))
  - feat: run AutoNAT V2 service in addition to V1 (#10468) ([ipfs/emo#10468](https://github.com/ipfs/emo/pull/10468))
  - feat: go-libp2p 0.36 and /webrtc-direct listener (#10463) ([ipfs/emo#10463](https://github.com/ipfs/emo/pull/10463))
  - chore: update dependencies (#10462)(#10466) ([ipfs/emo#10466](https://github.com/ipfs/emo/pull/10466))
  - feat: periodic version check and json config (#10438) ([ipfs/emo#10438](https://github.com/ipfs/emo/pull/10438))
  - docs: clarify pnet limitations
  - docs: "error mounting: could not resolve name" (#10449) ([ipfs/emo#10449](https://github.com/ipfs/emo/pull/10449))
  - docs: update ipfs-swarm-key-gen example (#10453) ([ipfs/emo#10453](https://github.com/ipfs/emo/pull/10453))
  - chore: update deps incl. boxo v0.21.0 (#10444) ([ipfs/emo#10444](https://github.com/ipfs/emo/pull/10444))
  - chore: go-libp2p 0.35.1 (#10430) ([ipfs/emo#10430](https://github.com/ipfs/emo/pull/10430))
  - docsa: update RELEASE_CHECKLIST.md
  - chore: create next changelog (#10443) ([ipfs/emo#10443](https://github.com/ipfs/emo/pull/10443))
  - Merge Release: v0.29.0 [skip changelog] ([ipfs/emo#10442](https://github.com/ipfs/emo/pull/10442))
  - fix(cli): unify --name param in ls and add (#10439) ([ipfs/emo#10439](https://github.com/ipfs/emo/pull/10439))
  - fix(libp2p): streams config validation in resource manager (#10435) ([ipfs/emo#10435](https://github.com/ipfs/emo/pull/10435))
  - chore: fix some typos (#10396) ([ipfs/emo#10396](https://github.com/ipfs/emo/pull/10396))
  - chore: update version
