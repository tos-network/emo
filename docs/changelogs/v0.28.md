...
The legacy subset of the Emo RPC that was available via the Gateway port and was deprecated is now completely removed. You can read more in <https://github.com/ipfs/emo/issues/10312>.

If you have a legacy software that relies on this behavior, and want to expose parts of `/api/v0` next to `/ipfs`, use reverse-proxy in front of Emo to mount both Gateway and RPC on the same port. NOTE: exposing RPC to the internet comes with security risk: make sure to specify access control via [API.Authorizations](https://github.com/ipfs/emo/blob/master/docs/config.md#apiauthorizations).

#### Removed deprecated Object API commands

The Object API commands deprecated back in [2021](https://github.com/ipfs/emo/issues/7936) have been removed, except for `object diff`, `object patch add-link` and `object patch rm-link`, whose alternatives have not yet been built (see issues [4801](https://github.com/ipfs/emo/issues/4801) and [4782](https://github.com/ipfs/emo/issues/4782)).

##### Emo ignores loopback addresses on LAN DHT and private addresses on WAN DHT

Emo no longer keeps track of loopback and private addresses on the LAN and WAN DHTs, respectively. This means that other nodes will not try to dial likely undialable addresses.

To support testing scenarios where multiple Emo instances run on the same machine, [`Routing.LoopbackAddressesOnLanDHT`](https://github.com/ipfs/emo/blob/master/docs/config.md#routingloopbackaddressesonlandht) is set to `true` when the `test` profile is applied.

#### Pin roots are now prioritized when announcing

The root CIDs of pinned content are now prioritized when announcing to the Amino DHT with [`Reprovider.Strategy`](https://github.com/ipfs/emo/blob/master/docs/config.md#reproviderstrategy) set to `all` (default) or `pinned`, making the important CIDs accessible faster.
...
- github.com/ipfs/emo:
  ...
  - chore: update version
  - core/node: prioritize announcing pin roots, and flat strategy (#10376) ([ipfs/emo#10376](https://github.com/ipfs/emo/pull/10376))
  - chore: webui v4.2.1 (#10391) ([ipfs/emo#10391](https://github.com/ipfs/emo/pull/10391))
  - docs(config): clarify RPC vs Gateway
  - chore: upgrade go-libp2p-kad-dht (#10378) ([ipfs/emo#10378](https://github.com/ipfs/emo/pull/10378))
  - chore(config): make Routing.AcceleratedDHTClient a Flag (#10384) ([ipfs/emo#10384](https://github.com/ipfs/emo/pull/10384))
  - fix: switch lowpower profile to autoclient
  - core: fix some typos (#10382) ([ipfs/emo#10382](https://github.com/ipfs/emo/pull/10382))
  - docs: fix some typos (#10377) ([ipfs/emo#10377](https://github.com/ipfs/emo/pull/10377))
  - core/commands!: remove deprecated object APIs (#10375) ([ipfs/emo#10375](https://github.com/ipfs/emo/pull/10375))
  - docs: update default ipns lifetime
  - coreapi/unixfs: don't create an additional IpfsNode for --only-hash
  - chore: cleanup old workaround (#10369) ([ipfs/emo#10369](https://github.com/ipfs/emo/pull/10369))
  - chore: finish reframe removal
  - docs: remove repetitive words (#10370) ([ipfs/emo#10370](https://github.com/ipfs/emo/pull/10370))
  - docs: updated links and refs to external resources (#10368) ([ipfs/emo#10368](https://github.com/ipfs/emo/pull/10368))
  - core/corehttp!: remove /api/v0 from gateway port
  - client/rpc!: remove deprecated DHT commands
  - ci: upgrade to go 1.22 (#10355) ([ipfs/emo#10355](https://github.com/ipfs/emo/pull/10355))
  - chore: create next changelog
  - Merge Release: v0.27.0 [skip changelog] ([ipfs/emo#10362](https://github.com/ipfs/emo/pull/10362))
  - test: cleanup content blocking tests (#10360) ([ipfs/emo#10360](https://github.com/ipfs/emo/pull/10360))
...
- github.com/libp2p/go-libp2p-kad-dht (v0.24.4 -> v0.25.2):
  ...
  - merge: fix: issues discovered in emo v0.21.0-rc2 (#853) ([libp2p/go-libp2p-kad-dht#853](https://github.com/libp2p/go-libp2p-kad-dht/pull/853))
  - merge: fix: issues discovered in emo v0.21.0-rc1 (#851) ([libp2p/go-libp2p-kad-dht#851](https://github.com/libp2p/go-libp2p-kad-dht/pull/851))
