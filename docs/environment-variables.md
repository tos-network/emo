...
Specifies the log level for Emo.
...
Sets the file to which Emo logs. By default, Emo logs to standard error.

## `GOLOG_TRACING_FILE`

Sets the file to which Emo sends tracing events. By default, tracing is
disabled.
...
Sets the file descriptor limit for Emo. If Emo fails to set the file
...
IPFS Content Path from which Emo fetches repo migrations (when the daemon
...
inspection/debug of HTTP requests sent by Emo via `while true ; do nc -l 7423; done`
...
Emo tries to reuse the same source port for all connections to improve NAT
...
Tells Emo which multiplexers to use in which order.
...
When set, overrides [`Swarm.ResourceMgr.Enabled`](https://github.com/ipfs/emo/blob/master/docs/config.md#swarmresourcemgrenabled) from the config.
